...
//            MLog.e("PARSE ERROR", parseExp.toString())
    ...
    companion object {
        ...
        fun millisToTime(millis: Long): String {
            Log.e("UtilTimes","millis: $millis")
            var seconds = 0
            var minutes = 0
            var hours = 0
            if (millis > 1000) {
                seconds = (millis / 1000).toInt()
                Log.e("UtilTimes","seconds: $seconds")
                if (seconds > 60) {
                    minutes = (seconds / 60)
                    Log.e("UtilTimes","minutes: $minutes")
                    seconds %= 60
                    if (minutes > 60) {
                        hours = minutes/60
                        Log.e("UtilTimes","hours: $hours")
                        minutes %= 60
                    }
                }
            }
            ...
            else if (hours == 0 && minutes == 0) "00:${if (seconds > 9) "$seconds" else "0$seconds"}"
            else ""
        }
    }
